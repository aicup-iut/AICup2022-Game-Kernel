...
class AICUP2022ENV(gym.Env):
    ...
    agent_sight_range=5
        ...
        self.observation_space = spaces.Box(0,1,(self.x_size,self.y_size),np.int16)
        self.reset()
    ...
    def manhattan_sight(self,x,y,distance=5,fog=False):
        ...
        r = (distance-1)//2
        temp_observation=np.zeros((distance,distance,4),dtype=int)+ OUT_OF_SIGHT
        for i in range(distance):
            for j in range(abs(i-r),distance-abs(i-r)):
                ...
                if self.check_coord_valid(temp_x,temp_y):
                    ...
                    temp_observation[i][j]=np.array([self.main_board[temp_x][temp_y],self.data_board[temp_x][temp_y],temp_x,temp_y],dtype=int)
                    if self.fog_map[temp_x][temp_y]==True and fog:
                        temp_observation[i][j]=[FOG,0,temp_x,temp_y]
                else:
                    temp_observation[i][j]=[OUT_OF_MAP,OUT_OF_SIGHT,OUT_OF_MAP,OUT_OF_MAP]
    ...
    def generate_observation(self):
        ...
        for agent in self.agents_list:
            x,y=agent.x,agent.y
            manhattan=self.manhattan_sight(x,y,self.agent_sight_range,fog=True)
            manhattan=manhattan.reshape(np.shape(manhattan)[0]*np.shape(manhattan)[1],np.shape(manhattan)[2])
            ...
            ,agent.alpha
            ,self.cool_down_rate
            ,min(self.cool_down_rate**agent.alpha,1)
            ,agent.def_lvl
            ,agent.atk_lvl
            ,agent.wallet
            ,agent.safe_wallet
            ,agent.action
            ,[i.wallet for i in self.agents_list]
            ]
    ...
    def generate_info(self):
        ...
        coin_map[np.where(self.main_board==GOLD)]=self.data_board[np.where(self.main_board==GOLD)]
        for agent in self.agents_list:
            info.append(
                {
                'winner':self.winner
                ,'win_reason' : self.win_reason
                ,'attack efficiency':self.cool_down_rate**agent.alpha
                ,'rounds':self.rounds
                ,'def_lvl':agent.def_lvl
                ,'atk_lvl':agent.atk_lvl
                ,'wallet':agent.wallet
                ,'safe wallet':agent.safe_wallet
                ,'x':agent.x
                ,'y':agent.y
                ,'board':coin_map
                ,'data':self.data_board
                ,'map' :self.main_board
                ,'action':agent.action
    ...
    def load_map(self):
        ...
        self.treasury_coord=self.initialize_treasury(conf_dic["treasury_coordinate"][0],conf_dic["treasury_coordinate"][1])
    ...
    def ranged_attack(self,attacker:agent):
        ...
        attack_x=attack_range[:,:,2][np.where(attack_range[:,:,1]!=OUT_OF_SIGHT)]
        attack_y=attack_range[:,:,3][np.where(attack_range[:,:,1]!=OUT_OF_SIGHT)]
        self.attack_board[attack_x,attack_y]=1
        #update attack list:
        agents_in_range=attack_range[np.where(attack_range[:,:,0]==AGENT)]
    ...
    def hit_agent(self,attacker:agent,defender:agent):
        ...
        gold=self.perimeter_gold_distribution(gold_amount,defender.x,defender.y)
        for i in range(3):
            for j in  range(3):
    ...
    def agents_random_order(self,action):
        ...
        s_key=lambda index:self.agents_list[index].wallet+priority_list[action[index]] if(action[index]<UPGRADE_DEFENCE)  else priority_list[action[index]]
    ...
    def decide_winner(self):
        ...
        self.win_reason=2
        team0_robb=self.agents_list[0].robbed_gold+self.agents_list[1].robbed_gold
        team1_robb=self.agents_list[2].robbed_gold+self.agents_list[3].robbed_gold
        ...
        self.win_reason=3
        team0_cost=self.agents_list[0].upgrade_cost+self.agents_list[1].upgrade_cost
        team1_cost=self.agents_list[2].upgrade_cost+self.agents_list[3].upgrade_cost
